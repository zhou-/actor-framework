...
public:
  ...
  behavior make_behavior() override {
    ...
    auto fwd = [=](scheduled_actor*, message& x) -> result<message> {
      send_to_acquaintances(std::move(x));
      return message{};
    };
...
behavior proxy_broker::make_behavior() {
  ...
  // instead of dropping "unexpected" messages,
  // we simply forward them to our acquaintances
  auto fwd = [=](local_actor*, message& x) -> result<message> {
    group_->send_all_subscribers(current_element_->sender, std::move(x),
                                 context());
    return message{};
  };
  set_default_handler(fwd);

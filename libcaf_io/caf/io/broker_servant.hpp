...
public:
  ...
  broker_servant(handle_type x)
    : hdl_(x),
      value_(make_message(SysMsgType{x, {}}), strong_actor_ptr{},
             make_message_id(), mailbox_element::forwarding_stack{}) {
    // nop
  }
...
protected:
  ...
  bool invoke_mailbox_element(execution_unit* ctx) {
    ...
    if (prev && activity_tokens_ && --(*activity_tokens_) == 0) {
      ...
      mailbox_element tmp{make_message(passiv_t{hdl()}), strong_actor_ptr{},
                          make_message_id(),
  ...
  SysMsgType& msg() {
    return value_.content.template get_mutable_as<SysMsgType>(0);
  }
